...
#include "stack/phy/layer/DataPath.h"

// #define OUTPUT_RAW_RSSI_DATA_PATH
// "/home/jkmxm/work/2023/vmm-ipv4-omnetpp/data/raw_rssi_data.csv" #define
// OUTPUT_RAW_RSSI_DATA_PATH "../../data/raw_rssi_data.csv"
...
protected:
  ...
  /**
   * Hysteresis threshold to evaluate handover: it introduces a small
   * polarization to avoid multiple subsequent handovers
   */
  ...
  /**
   * Time interval elapsing from the reception of first handover broadcast
   * message to the beginning of handover procedure. It must be a small number
   * greater than 0 to ensure that all broadcast messages are received before
   * evaluating handover. Note that broadcast messages for handover are always
   * received at the very same time (at bdcUpdateInterval_ seconds intervals).
   */
  // TODO: bring it to ned par!
  ...
  /** set to false if a battery is not present in module or must have infinite
   * capacity */
  bool useBattery_;
  ...
  virtual void finish(cComponent *component,
                      omnetpp::simsignal_t signalID) override {
  ...
  /**
   * Utility function to update the hysteresis threshold using
   * hysteresisFactor_.
   */
...
public:
   ...
   */
  virtual void sendFeedback(LteFeedbackDoubleVector fbDl,
