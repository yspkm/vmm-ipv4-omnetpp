...
void initialize_raw_data_file(char* file_path){
    // Hand over Data
    std::remove(file_path);
    std::ofstream outFile(file_path);
    if (!outFile.is_open()) {
        std::cerr << "Failed to create the file: " << file_path << std::endl;
        exit(1);
    }
    outFile.close();
}
...
LtePhyUe::LtePhyUe()
{
    ...
    initialize_raw_data_file(OUTPUT_RAW_RSSI_DATA_PATH);
}
...
#include <fstream> // midterm-2017312605-YosepKim
...
void LtePhyUe::triggerHandover()
{
    ...
    //TODO_2023-2_10000_LINE_PROJECT” : {simTime, currentMasterId, nextMasterId} *simTime is HO complete time.
   ...
   /*
    int ueIndex = nodeId_ - 2048;
    std::string outputString = std::to_string(int(simTime().dbl())) + ", " + std::to_string(ueIndex) + ", " + std::to_string(rssi) + "\n";
    std::ofstream outputFile(OUTPUT_RAW_RSSI_DATA_PATH, std::ios_base::app);
    outputFile << outputString;
    outputFile.close();
    */
    ...
    //if (int(NOW.dbl())!=0){
        //int ueIndex = nodeId_ - 2048;
        //std::string outputString = std::to_string(int(NOW.dbl())) + ", " + std::to_string(ueIndex) + ", " + std::to_string(masterId_) + ", " + std::to_string(candidateMasterId_) + "\n";
        //std::ofstream outputFile(OUTPUT_RAW_HO_DATA_PATH, std::ios_base::app);
        //outputFile << outputString;
        //outputFile.close();
    //}
